 (function($) { 
	...
	jQuery(document).on("change", "select[name=pipeliner_module]", function(){
		
		jQuery('.loader').addClass('is-active');

		pipeliner_module_name = jQuery(this).val();

		jQuery.ajax({
	        url: smart_pipeliner_js.ajaxurl,
	        type: 'post',
	        data: {
	            'action':'pipeliner_field',
	            'pipeliner_module_name': pipeliner_module_name
	        },
	        success: function( response ) {
	            jQuery("select[name=pipeliner_field]").empty().append(response);
	            jQuery('.loader').removeClass('is-active');
	        },
	    });
	});
